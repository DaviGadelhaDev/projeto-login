    ...
    function Admin() {
        ...
        async function handleRegister(e) {
            ...
            await addDoc(collection(db, "tarefas"), {
                tarefa: tarefaInput,
                created: new Date(), //saber quando foi criada a tarefa
                userUid: user?.uid //o ? serve para caso venha vazio ele não irá crachar a aplicação
            })
                .then(() => {
                    console.log('Tarefa rgistrada')
                    setTarefaInput('')
                })
        ...
        async function deleteTarefa(id){
            const  docRef = doc(db, "tarefas", id) //qual tarefa vc quer deletar por isso passamos o id
            ...
            <div className='admin-container'>
                ...
                {tarefas.map((item) => (
                    <article key={item.id} className='list'>
                        ...
                        <div>
                            <button onClick={() => editTarefa(item)}>Editar</button> {/*passar qual o item que vc está clicando */}
